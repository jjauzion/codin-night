...
def count(phrase, l_word, index):
    ...
    if phrase_start_with(phrase, l_word[index]):
        sub_phrase = phrase[len(l_word[index]):]
        l_sub_word = [word for word in l_word if word in sub_phrase]
        cpt += count(sub_phrase, l_sub_word, 0)
...
if __name__ == "__main__":
    ...
    print("nb of word = {}".format(len(dico)))
    l_word = [word_to_morse(word) for word in dico if word_to_morse(word) in l]
    print("{}\n{}".format(dico, l_word))
    print("nb of word = {}".format(len(l_word)))
